...
# Флоу работы
Роутер определяет `intent` пользователя - требует ли запрос тяжелых перерасчетов с участием генерации кода или это уточняющий вопрос.  
Планировщик зациклен на верификатор для улучшения плана расчетов.  
Генаратор кода зациклен на интерпретатор Python для фикса ошибок в коде.  
